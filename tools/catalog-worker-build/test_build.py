...
CRITICAL = {"materialize.py", "worker.py", "supervisor.py", "Dockerfile", ".dockerignore", "requirements.lock", "test_materialize.py", "test_supervisor.py", "image-inputs.json", "inventory_census.py", "test_inventory_census.py", "coverage_report.py", "test_coverage_report.py", "test_inventory_integration.py", "repair_plan.py", "repair_runtime.py", "repair_supervisor.py", "test_repair_plan.py", "test_repair_runtime.py"}
...
class ValidateBuildTests(unittest.TestCase):
    ...
    def test_rejects_incomplete_or_extended_critical_input_set(self):
        with tempfile.TemporaryDirectory() as raw:
            directory = Path(raw); _, build = receipts(directory)
            build["source_files_sha256"].pop("repair_runtime.py")
            write_json(directory / "receipt.json", build, compact=True)
            (directory / "receipt.sha256").write_text(hashlib.sha256((directory / "receipt.json").read_bytes()).hexdigest() + "\n", encoding="ascii")
            self.assertNotEqual(validate(directory).returncode, 0)
